...
exports.createItemWarehouse = async (req, res) => {
  ...
  try {
    const { id_item, id_almoxarifado, quantidade, data_entrada, observacao } =
      req.body;
      ...
      await Movement.create(
        {
          ...
          quantidade,
          observacao:
            observacao || 'Entrada inicial - item adicionado ao almoxarifado',
          id_item,
          id_almoxarifado,
          id_usuario,
        },
...
exports.updateItemWarehouse = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      novaQuantidade !== itemWarehouse.quantidade
    ) {
      ...
      await Movement.create(
        {
          ...
          observacao:
            observacao ||
            `${diferenca > 0 ? 'Entrada' : 'Saída'} de estoque: ${Math.abs(diferenca)} unidades`,
      ...
      await Movement.create(
        {
          ...
          observacao:
            observacao || 'Saída total - vínculo item-almoxarifado removido',
          id_item: itemWarehouse.id_item,
          id_almoxarifado: itemWarehouse.id_almoxarifado,
          id_usuario,
        },
